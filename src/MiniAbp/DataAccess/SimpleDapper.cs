...
#pragma warning restore 414
            ...
            /// <param name="commandTimeout"></param>
            /// <returns></returns>
            public static PagedList<T> GetListPaged<T>(this IDbConnection connection, object whereCondition, IPaging page,  IDbTransaction transaction = null, int? commandTimeout = null)
            {
                var currenttype = typeof(T);
                var idProps = GetIdProperties(currenttype).ToList();
                if (!idProps.Any())
                ...
                var whereprops = GetAllProperties(whereCondition).ToArray();
                if (whereprops.Any())
                {
                    sb.Append(" WHERE ");
                    BuildWhere(sb, whereprops, (T)Activator.CreateInstance(typeof(T)), whereCondition);
                }

                if (Debugger.IsAttached)
                    Trace.WriteLine(String.Format("GetListPaged<{0}>: {1}", currenttype, sb));


                var sqlWithPaged = BuilderPageSql(sb.ToString(), page.OrderByProperty, !page.Ascending, page.CurrentPage,
                    page.PageSize);
            ...
            public static TKey Insert<TKey>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
            {
                ...
                if (keytype != typeof(int) && keytype != typeof(uint) && keytype != typeof(long) && keytype != typeof(string) && keytype != typeof(ulong) && keytype != typeof(short) && keytype != typeof(ushort) && keytype != typeof(Guid))
                {
                ...
                if ((keytype == typeof(int) || keytype == typeof(long)) && (Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0 || Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0))
                {
            ...
            public static int Count<T>(this IDbConnection connection, string conditions = "", IDbTransaction transaction = null, int? commandTimeout = null)
            {
                var currenttype = typeof(T);
                var name = GetTableName(currenttype);
                var sb = new StringBuilder();
            ...
            private static void BuildInsertValues(object entityToInsert, StringBuilder sb)
            {
                ...
                for (var i = 0; i < props.Count(); i++)
                {
                    ...
                    if (property.Name == "Id" && property.GetCustomAttributes(true).All(attr => attr.GetType().Name != "RequiredAttribute") && property.PropertyType != typeof(Guid) && property.PropertyType != typeof(string)) continue;
            ...
            {
                var trimedSql = sqlStr.Trim();
                if (trimedSql.ToUpper().StartsWith("SELECT"))
                {
                    return trimedSql.Substring(6, trimedSql.Length - 6);
                }
                throw new NotSupportedException("Please make sure the query sql scrit started with 'SELECT'.");
            }
            #endregion
